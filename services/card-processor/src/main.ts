...
import type { BaseEnv, KafkaEnv } from '@libs/config';
import { AppModule } from './app.module';

type AppConfig = BaseEnv & KafkaEnv;
...
async function bootstrap() {
  ...
  const configService = tempApp.get<ConfigService<AppConfig, true>>(ConfigService);
  ...
  logger.log('🔔 Card processor microservice is listening for Kafka events');
